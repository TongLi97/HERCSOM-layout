from numpy import (array, unravel_index, nditer, linalg, random, subtract, max, log, finfo,histogram,
                   power, exp, zeros, ones, arange, outer, meshgrid, dot, newaxis,
...
"""
    Minimalistic implementation of the homology-enhanced self-organizing map (HERC-SOM).
"""
...
class MiniSom(object):
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  resource_limit, labels_encoding, alpha
    def __init__(self, x, y, input_len, resource_limit=None,
                 data=None, labels_encoding=None, alpha=None,
        ...
        resource_limit : int
            Number of resources allocated to each neuron.
            This parameter determines the number of samples in each neuron.

        data: np.array   shape(n, d)
            Sample data for SOM input.
            n: samples Number
            d: Vector Dimensions

        labels_encoding : np.array  shape(n, c)
            Data Category matrix.
            Category information for each sample.
            n: samples number
            d: categories number (or number of one hot encoding bits)

        alpha: float
            Adjust the balance parameter of the joint result of weight vector and probability distribution
        # >>>>>>>>>>>>>>>>>>>>>>>>
        ...
        self._activation_counts = zeros((x, y))
        self._data = data
        self._labels_encoding = labels_encoding
        self._labels_distributions = ones((x, y, len(labels_encoding[0]))) * (1 / len(labels_encoding[0]))
        self._alpha = alpha
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    def _activate(self, x):
        ...
        feature_distances = self._activation_distance(x, self._weights)
        x_idx = where((self._data == x).all(axis=1))[0][0]
        x_label = self._labels_encoding[x_idx]
        label_similarities = dot(self._labels_distributions, x_label)
        self._activation_map =  (1 - self._alpha) * feature_distances + self._alpha * (1 - label_similarities)
    ...
    def update(self, x, win, t, max_iteration):
        ...
        g_expanded = g[:, :, newaxis]  # Shape becomes (x, y, 1)

        # Update class distributions
        idx = where((self._data == x).all(axis=1))[0][0]
        x_label = self._labels_encoding[idx]  # Retrieve label for the index

        self._labels_distributions += eta * g_expanded * (x_label - self._labels_distributions)
    ...
    def train(self, data, num_iteration,
        ...
        for t, iteration in enumerate(iterations):
            ...
            bmu_idx = self.winner(data[iteration])
            self._activation_counts[bmu_idx] += 1
            self.update(data[iteration], bmu_idx, decay_rate, num_iteration)
    ...
    def activation_response(self):
        ...
        By calculating the entropy value of the activation_counts (a matrix where neurons have been winner).
        """
        # Flatten the array to include all activations in the entropy calculation
        flat_activations = self._activation_counts.flatten()
        # Calculate the probability distribution of activations
        probabilities, _ = histogram(flat_activations, bins=range(int(flat_activations.min()), int(flat_activations.max()) + 2), density=True)
        # Calculate entropy
        activation_entropy = entropy(probabilities)

        return activation_entropy, self._activation_counts


    def distribution_error(self):
        """Computes the category distribution error as the mean Kullback-Leibler divergence
        between each input sample's category encoding and its best matching unit's category probability distribution."""
        self._check_input_len(self._data)
        errors = []
        self._resource_initialize()
        for sample, label in zip(self._data, self._labels_encoding):
            bmu = self.winner(sample)  # Find the best matching unit for each sample
            bmu_distribution = self._labels_distributions[bmu[0], bmu[1]]
            # Calculate the KL divergence, ensure no zero probability issue
            kl_divergence = entropy(label + 1e-15, bmu_distribution + 1e-15)
            errors.append(kl_divergence)
        return mean(errors)  # Return the mean error over all samples
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
